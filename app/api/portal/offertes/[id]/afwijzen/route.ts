...
export async function POST(
  ...
  try {
    ...
    clientNotes = typeof body.notes === "string" ? body.notes.trim() : "";
  } catch {
    clientNotes = "";
  }
  ...
  if (!clientNotes) {
    return NextResponse.json(
      { error: "Een toelichting is verplicht bij het afwijzen." },
      { status: 422 },
    );
  }
  ...
  if (offerte.status !== "verstuurd") {
    return NextResponse.json(
      { error: "Deze offerte kan niet meer worden afgewezen." },
      { status: 409 },
    );
  }
  ...
  await prisma.$transaction([
    prisma.offerte.update({
      where: { id: offerteId },
      data: { status: "afgewezen", rejectedAt: new Date(), clientNotes },
    }),
  ...
  if (adminEmail) {
    ...
    await resend.emails.send({
      ...
      to: adminEmail,
      subject: `Offerte ${offerte.nummer} afgewezen door ${offerte.klant.name}`,
  ...
  return NextResponse.json({ success: true, status: "afgewezen" });
}
